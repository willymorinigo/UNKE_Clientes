...
export default function ProjectsTracker({
  ...
  const handleUpdateStatus = async (project: Project, newStatus: ProjectStatus) => {
    ...
    try {
      ...
      // WhatsApp trigger on budget approval!
      if (newStatus === "approved") {
        setTimeout(() => {
          const confShare = window.confirm("¿Deseas enviar los detalles y el enlace del presupuesto aprobado al grupo de WhatsApp del estudio?");
          if (confShare) {
            handleShareToWhatsApp(project);
          }
        }, 400);
      }
    } catch (err) {
  ...
  // Share project details directly to WhatsApp
  const handleShareToWhatsApp = (project: Project) => {
    const matchedClient = clients.find(c => c.id === project.clientId);
    const clientName = matchedClient?.name || project.clientName;
    const clientEmail = matchedClient?.email || project.clientEmail;

    // Build the list of pieces
    let piecesText = "";
    if (project.pieces && project.pieces.length > 0) {
      piecesText = project.pieces.map(p => 
        `• *${p.pieceName}* (x${p.quantity}) - Comp: ${p.complexity === "low" ? "Baja" : p.complexity === "medium" ? "Media" : "Alta"} [${formatARS(p.price)}]`
      ).join("\n");
    } else {
      piecesText = `• *${project.pieceType || "Pieza única"}* (x${project.quantity}) - Comp: ${project.complexity === "low" ? "Baja" : project.complexity === "medium" ? "Media" : "Alta"}`;
    }

    // Detail text of drive or attachment instructions
    let driveText = "";
    if (project.driveFileUrl) {
      driveText = `\n📂 *Enlace a Google Drive:* ${project.driveFileUrl}`;
    } else {
      driveText = `\n📄 _Nota: Descargar el PDF del panel y adjuntar el archivo al grupo._`;
    }

    const notesText = project.customNotes ? `\n📝 *Indicaciones:* ${project.customNotes}` : "";

    // Generate full text
    const message = `*🟢 ¡PRESUPUESTO APROBADO! - UNKE Estudio Creativo*

*Proyecto:* ${project.name}
*Cliente:* ${clientName} (${clientEmail})
*Monto Total:* ${formatARS(project.price)}
${notesText}
...
_Enviado desde UNKE Clientes_`;

    const whatsappUrl = `https://api.whatsapp.com/send?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, "_blank", "noopener,noreferrer");
    
    onLogAction("SHARE_WHATSAPP", `Compartió presupuesto de de '${project.name}' en WhatsApp.`);
  };
                    ...
                    <div key={proj.id} className="bg-white border border-neutral-200 hover:border-neutral-400 rounded-md p-3 flex flex-col justify-between space-y-3 shadow-none transition animate-fadeIn">
                      ...
                      <div className="flex flex-col gap-1.5 pt-0.5">
                        ...
                        <div className="flex gap-1 w-full">
                          ...
                          <button
                            onClick={() => handleShareToWhatsApp(proj)}
                            className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-1 px-2 rounded text-[10px] font-semibold flex items-center justify-center gap-1 transition-colors cursor-pointer"
